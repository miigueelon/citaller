...
- [x] 0.5a CLIs instaladas: Supabase CLI 2.117 (devDependency, commit `f4ec8aa`), Vercel CLI 59 (global). Google Cloud SDK y Deno: instalación por winget en curso.
- [x] 0.6a `npx supabase init` hecho (commit `161eb5c`). `supabase db dump` usa Docker → backup por `pg_dump` (winget) o `scripts/backup.mjs` con `pg`, decisión al vincular.
