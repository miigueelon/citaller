...
| `v1.3` | Lo de `v1.2` más "Vehículo listo" termina la cita (cabecera "Hoy: N por terminar", marca "✓ Lista" con Deshacer) y los avisos de WhatsApp quedan apuntados en la tarjeta (22-sep-2026, madrugada). Pendiente de publicar | `backups/2026-09-21_2240` (las dos migraciones de `v1.3` solo añaden una columna y dos funciones) |
...
- Desde el 22-sep-2026, al pulsar cualquiera de esos botones la cita queda **apuntada como avisada** ("✓ Confirmación avisada a las 12:30", "✓ Recordatorio enviado…"; RPC `marcar_aviso_whatsapp`, mismas columnas `whatsapp_*_enviada/_fecha` que usa el modo api). El botón sigue disponible como "Volver a avisar". Si el móvil abre WhatsApp pero la marca no se guarda, el panel lo dice y basta con actualizar.
- **"Vehículo listo"** (citas confirmadas de hoy o anteriores): abre WhatsApp con "ya puedes recogerlo" y **termina la cita** (`reservas.listo_en`, RPC `marcar_vehiculo_listo`). La cabecera pasa a "Hoy: N por terminar" y, con todas, "Hoy: todo terminado"; la tarjeta muestra "✓ Lista · avisado a las HH:MM" con **Deshacer** (por si se pulsó en la que no era). Una cita terminada ya no ofrece "Cancelar cita"; sigue siendo Confirmada para la capacidad, el historial y el enlace del cliente.
