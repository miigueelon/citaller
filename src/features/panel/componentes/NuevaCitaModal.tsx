 ...
 * El taller puede elegir cualquier hora: si está fuera del horario o está llena, se avisa pero no
 * se bloquea; una hora de hoy que ya ha pasado sí se bloquea (pedido del 24-sep). Si el taller
 * tiene miembros, hay que escoger quién la apunta en cada cita (no se recuerda el anterior). Con
 * `exigirTodo` (decisión de cada taller) el teléfono, el primer apellido y la descripción son
 * obligatorios; en los desplegables se ofrecen las opciones del mostrador (`opciones_panel`), que
 * pueden ser más que las del público.
 */
export function NuevaCitaModal({ miembros, exigirTodo, ocupado, onGuardar, onCerrar }: Props) {
  ...
  const miembroId = preguntarMiembro ? datos.miembro_id : null;
  ...
  // Se recalcula al pulsar Guardar: "la hora actual" cambia mientras el formulario está abierto.
  const calcularFaltan = () => datosQueFaltan(datos, { exigirTodo, preguntarMiembro, miembroId, servicio, campos });
  ...
  async function guardar() {
    ...
    const faltanAhora = calcularFaltan();
    if (faltanAhora.length > 0) {
      setError(`Falta: ${faltanAhora.join(", ")}.`);
      return;
    }
    setError(null);
