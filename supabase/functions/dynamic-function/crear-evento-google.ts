...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Evitar crear el mismo evento dos veces
    if (reserva.google_event_id) {
      return new Response(
        JSON.stringify({
          ...
          evento_creado: false,
          mensaje:
            "Esta reserva ya tiene evento en Google Calendar",
          google_event_id:
    ...
    if (
      ...
      return new Response(
        JSON.stringify({
          ok: false,
          error:
            "No tienes permiso para esta reserva",
        }),
        {
          status: 403,
          headers: {
    ...
    // ========================================
    // CONEXIÓN GOOGLE DEL TALLER
    // ========================================

    const {
      data: integracion,
      error: integracionError,
    } = await supabase
    ...
    if (!tokenResponse.ok) {
      ...
      return new Response(
        JSON.stringify({
          ok: false,
          error:
            "No se pudo renovar el acceso a Google Calendar",
        }),
        {
          status: 502,
          headers: {
    ...
    // De momento usamos 60 minutos.
    // Más adelante podemos usar duración
    // distinta según el servicio.
    ...
    // Google events.insert devuelve el evento
    // creado, incluido su id/htmlLink.
    ...
    return new Response(
      JSON.stringify({
        ...
        evento_creado: false,
        error:
          "Error interno creando el evento",
      }),
      {
        status: 500,
        headers: {
