...
- [x] 0.5b Logins: Supabase (token) y proyecto vinculado; Google (`miguel.rodriguez.sevilla93@gmail.com`, proyecto `citaller-508917`, Calendar API habilitada). Pendientes: Vercel (`vercel login` en curso) y **contraseña de la base de datos** para `db dump`/`db push`.
...
- [ ] 0.6b Backup + migración baseline: `supabase db dump` requiere Docker; alternativa `pg_dump` vía winget o `scripts/backup.mjs` con `pg`. Necesita la contraseña de la BD.
...
- [ ] 0.8b Repetir `scripts/rls-test.sql` al cierre de cada fase y comparar con `docs/rls-baseline.md`.
